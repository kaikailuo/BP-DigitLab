...
def _foreground_mask_from_gray_debug(
...
def preprocess_canvas_image(canvas_image: np.ndarray) -> Dict[str, Any]:
    gray_0_255 = _extract_canvas_gray(canvas_image)
    pil = Image.fromarray(gray_0_255.astype(np.uint8), mode="L")
    tensor = _normalize_28x28(pil)
    return {
        "canvas_gray": gray_0_255,
        "final_28": tensor.squeeze(0).cpu().numpy(),
        "tensor": tensor,
    }


def preprocess_uploaded_image(image: Image.Image) -> Dict[str, Any]:
